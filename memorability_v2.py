...
Feature streams:
  ...
  2. LLM semantic scalars         — 8 task-specific scores via OpenAI
  3. Brand entity density         — 8 string-matching features from STT + metadata
  4. Pre-extracted CNN features   — 7 architectures, PCA-32

Model:
  - Random Forest (500 trees), annotation-count sample weighting
...
# ══════════════════════════════════════════════════════════════════════════════
# DIRECTION 4 — Pre-extracted CNN features (PCA-32)
# ══════════════════════════════════════════════════════════════════════════════
...
def load_cnn_features(df: pd.DataFrame,
    ...
    """
    Load 7 pre-extracted CNN/Transformer feature files per video.
    Temporal (T, D) arrays are collapsed with mean+std concatenation.
    All models are concatenated then reduced to PCA_DIM components.

    Training mode  (pca_model=None): fits PCA+scaler, returns (X, pca, scaler).
    Inference mode (models provided): transforms only, returns same tuple.
    """
    all_mats = []
    ...
    if not all_mats:
        ...
        return np.zeros((len(df), 0)), pca_model, scaler_model
...
def extract_handcrafted_features(df: pd.DataFrame,
    ...
    """Run directions 1, 2, 3 for every row. Returns (frame_df, brand_df, llm_df)."""
    ...
    for i, (_, row) in enumerate(df.iterrows()):
        ...
        llm_rows.append(get_llm_scalars(vid_id, row, stt_text,
                                         cache, openai_client))
...
def train_final_models(X_mem: np.ndarray, X_brand: np.ndarray,
    ...
    """
    Train separate RF models (500 trees) per target on the full dataset.
    ...
    mdl_mem      = RandomForestRegressor(n_estimators=500, n_jobs=-1,
                                         random_state=RANDOM_SEED)
    mdl_mem.fit(scaler_mem.fit_transform(X_mem), y_mem, sample_weight=weights)

    scaler_brand = StandardScaler()
    mdl_brand    = RandomForestRegressor(n_estimators=500, n_jobs=-1,
                                         random_state=RANDOM_SEED)
...
def main():
    ...
    # ── Feature extraction ────────────────────────────────────────────────────
    log.info("Extracting handcrafted features (dirs 1, 2, 3) ...")
    ...
    log.info("Loading CNN features (dir 4) ...")
    ...
    scaler_mem, scaler_brand, mdl_mem, mdl_brand = train_final_models(
        full_X, X_brand_final, y_mem, y_brand, weights)
    ...
    log.info(f"  Brand model : CNN-only       (dim={X_brand_final.shape[1]})")
