...
Feature streams:
  ...
  3. LLM semantic scalars         — 20 task-specific scores via OpenAI
                                    (12 video-mem dims + 8 brand-mem dims)
...
_LLM_SYSTEM = (
    "You are an expert in advertising psychology, cognitive science, and "
    "video memorability research. Given a commercial video's metadata and "
    "transcript excerpt, score it on 20 dimensions that predict how well "
    "viewers will remember (a) the video itself and (b) which brand it was for. "
    "Return ONLY a valid JSON object — no markdown, no commentary, no extra keys."
)

_LLM_DIMENSIONS = {
    # ── Video memorability (12 dims) ──────────────────────────────────────────
    # Cognitive research shows these drive whether a viewer remembers seeing
    # this video at all (irrespective of brand).
    "emotional_valence":    "Overall emotional tone: 0=cold/alarming/negative → 10=warm/uplifting/positive",
    "narrative_arc":        "Clear story structure (problem→solution→resolution)? 0=none, 10=strong arc",
    "novelty_surprise":     "How unexpected or counter-intuitive is the message? 0=very predictable, 10=surprising",
    "hook_strength":        "How compelling is the opening (first ~10 sec)? 0=slow/bland, 10=immediately gripping",
    "human_presence":       "Presence of relatable human characters or personal stories: 0=none, 10=central",
    "pacing_energy":        "Perceived pace and energy: 0=slow/static talking-head, 10=fast/high-energy/dynamic",
    "concrete_imagery":     "Specific vivid imagery vs abstract concepts in narration: 0=very abstract, 10=very concrete",
    "humor_level":          "Presence of wit, irony, or humor: 0=none/serious, 10=strongly humorous",
    "emotional_peak":       "Presence of a distinct emotional climax or payoff moment: 0=flat, 10=strong peak",
    "personal_relevance":   "How relevant is the content to the everyday viewer's life? 0=irrelevant, 10=highly relatable",
    "message_clarity":      "How clear and simple is the single core message? 0=confusing/many messages, 10=crystal clear",
    "sensory_richness":     "Richness of sensory/descriptive language in transcript: 0=dry/factual, 10=vivid/evocative",

    # ── Brand memorability (8 dims) ───────────────────────────────────────────
    # These predict whether the viewer remembers *which brand* the video was for,
    # independently of whether they remember the video itself.
    "brand_prominence":     "How centrally/repeatedly is the brand name featured overall? 0=never, 10=constant",
    "brand_name_repetition":"How frequently is the brand name spoken in the transcript? 0=never, 10=very often",
    "brand_open":           "Is the brand name/logo introduced in the first 10 seconds? 0=no, 10=yes, prominently",
    "brand_close":          "Is the brand name/logo the final impression (last 10 sec)? 0=no, 10=yes, prominently",
    "brand_integration":    "How organically is the brand woven into the narrative (vs. bolted-on logo)? 0=afterthought, 10=inseparable",
    "brand_specificity":    "Concrete products/numbers/differentiators vs. vague positioning: 0=vague, 10=very specific",
    "call_to_action":       "How explicit and direct is the call to action? 0=none, 10=very direct",
    "brand_vs_category":    "Focus on this specific brand identity vs. generic category content: 0=generic category, 10=strongly brand-specific",
}
...
def _build_llm_prompt(row: pd.Series, stt_text: str) -> str:
    ...
    return (
        ...
        f"Score this commercial on 20 dimensions (all floats 0.0–10.0):\n"
        f"{schema_lines}\n\n"
        f"Return ONLY this JSON (no extra text, exactly these 20 keys):\n"
        f"{keys_json}"
    )
...
def get_llm_scalars(video_id: str, row: pd.Series, stt_text: str,
    ...
    """
    Query OpenAI for 20 task-specific semantic scalars with disk-backed caching.
    Cache key is prefixed with 'v2:' so old 8-scalar entries are ignored
    automatically without deleting the cache file.
    response_format=json_object guarantees valid JSON output.
    Falls back to neutral 5.0 values if no API key is available.
    """
    cache_key = f"v2:{video_id}"
